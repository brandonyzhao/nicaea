...
#define Nhod_t 6
typedef enum {hod_none, hamana04, berwein02, berwein02_hexcl, leauthaud11, coupon15} hod_t;
#define shod_t(i) (		\
 i==hod_none  ? "hod_none" :	\
 i==hamana04  ? "hamana04" :	\
...
typedef struct {
  ...
  hod_t hod;                    /* HOD type                                        */
  double log10M1, log10M0, log10M_min, sigma_log_M;
  double alpha;
  double pi_max;  
  ...
  /* For leauthaud11 and coupon15 models                                                         */
  double beta, delta, gamma, log10Mstar0;
  double beta_sat,B_sat,beta_cut,B_cut;
  double x;                     /* any parameter to propagate if needed            */
  double log10Mstar_min, log10Mstar_max;
  double fcen1, fcen2;
...
typedef struct {
  ...
  double  logMlim, bias_fac, Mh, log10Mstar, log10Mstar_min, log10Mstar_max;
  double  M, r_vir, *kk;
  error    **err;
...
cosmo_hm* init_parameters_hm(double OMEGAM, double OMEGADE, double W0_DE, double W1_DE,
			     ...
			     double log10M_min, double log10M1, double log10M0, double sigma_log_M, double alpha,
			     double log10Mstar0, double beta, double delta, double gamma, double B_cut, double B_sat, 
			     double beta_cut, double beta_sat, double log10Mstar_min, double log10Mstar_max, double eta,
...
double DeltaSigma_WB2000(cosmo_hm *model, double r, const double a, const double M,  double c, error **err);
