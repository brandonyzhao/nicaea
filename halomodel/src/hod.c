 ...
 * Refs:						                                                *
 *   - Coupon et al. 2015 (MNRAS in press)                          *
 ...
 * ---------------------------------------------------------------- */
{
  ...
  /* ngal fit not used in leauthaud11 or coupon15 models */
  testErrorExit(model->hod == leauthaud11 && ngal_fit_type != ngal_no_fit, HOD_NGAL_FIT,
	       "ngal fit cannot be used with leauthaud11 or coupon15 model; set \"sngal_fit_type ngal_no_fit\" and use stellar mass function instead",
	       *err, __LINE__);
  ...
  ngal_model = ngal_den(model,1.0/(1.0+zm), logMmax, model->log10Mstar_min, model->log10Mstar_max, err);   
  forwardError(*err, __LINE__, -1.0);
  ...
  if(ngal_fit_type == ngal_lin_fit_only){
    ...
    testErrorRet(!isfinite(chi2), ce_infnan, "inf or nan chi^2", *err, __LINE__, -1.0);
    return -0.5*chi2;
  }
  ...
  case smf :
    ...
    y_model = dNdlog10Mstellar(model, x, data->nbins, err);  forwardError(*err, __LINE__, -1.0);
    free(x);
    break;
  }
  ...
  if (data->wcov == NULL){
     ...
     for (i=0; i < data->nbins; i++) {
        for (j=0; j < data->nbins; j++) {
           ...
           testErrorRet(!isfinite(data->wcov[i*data->nbins+j]), ce_infnan, "inf or nan in logl", *err, __LINE__, -1.0);
        }
     }
  }
  free(y_model);
  ...
  */
  ...
  testErrorRet(!isfinite(chi2), ce_infnan, "inf or nan chi2", *err, __LINE__, -1.0);
 ...
 * ---------------------------------------------------------------- */
{
  ...
  for(i=0;i<Ntheta;i++){ /* loop over theta */
	...
	if (log(r) < logu[N-1]) {
	  /* to ensure log(r) lies within interpolation limits */
    ...
    testErrorRetVA(!isfinite(result[i]), ce_infnan, "inf or nan in w(theta_%d=%g)",
...
/* ---------------------------------------------------------------- *
 * Lensing functions (only for leauthaud11 and coupon15 model)                   *
 * ---------------------------------------------------------------- */
 ...
 * ---------------------------------------------------------------- */
{
  ...
  if(pofk == pstellar){
    if (model->hod == leauthaud11 || model->hod == coupon15){
      ...
      */
      if (model->log10Mstar_min < 0 || model->log10Mstar_max < 0) {
	Mstellar_mean = 0;
      } else {
	Mstellar_mean = pow(10.0, (MAX(model->log10Mstar_max, model->log10Mstar_min) + model->log10Mstar_min)/2.0);
      }
    }
...
/* ---------------------------------------------------------------- *
 * Stellar mass function (only for leauthaud11 or coupon15 models)  *
 * ---------------------------------------------------------------- */

double *dNdlog10Mstellar(cosmo_hm *model, double *log10Mstar, int N, error **err)
{
...
double *dNdlog10Mstellar_c(cosmo_hm *model, double *log10Mstar, int N, error **err)
{
  /* Computes the stellar mass function. See Leauthaud et al. (2011) eq. 17 
   * and Coupon et al. (2012) Eq. A. 21.
   * In h^3 Mpc^-3 dex^-1
...
double *dNdlog10Mstellar_s(cosmo_hm *model, double *log10Mstar, int N, error **err)
{
  /* Computes the stellar mass function. See Leauthaud et al. (2011) eq. 17 
   * and Coupon et al. (2012) Eq. A. 21.
   * In h^3 Mpc^-3 dex^-1
...
double *xi_1hcs(cosmo_hm *model,double a, const double *r, int N, int type, error **err){
  ...
  params.model  = model;
  params.a      = a;
  params.ng     = ngal_den(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, NULL);
  params.err    = err;
  params.eps    = 1.0e-4;
  ...
  for(i=0;i<N;i++){
    if(r[i] > RMAX1){
      ...
      // DEBUGGING
      //result[i] = int_gsl(int_for_xi_1hcs, (void*)&params, logMmin,
      //		 logMmax, params.eps, err);
 ...
 */
{
  ...
  case GG:
    ...
    res *= Ngal_c(model, M, model->log10Mstar_min, model->log10Mstar_max, err)*
      Ngal_s(model, M, model->log10Mstar_min, model->log10Mstar_max, err)/(0.5*ng*ng)*rho/M;
    forwardError(*err, __LINE__, 0.0);  
    break;
  case GM:
    ...
    res  *= Ngal_c(model, M, model->log10Mstar_min, model->log10Mstar_max, err)/ng * rho/(rhocrit*Omega_m);
    forwardError(*err, __LINE__, 0.0);
    break;
  }
...
double int_for_P1hss(double logM, void *params, error **err)
{
  ...
  double Ngal_sat = Ngal_s(model, M, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double concentration_sat(cosmo_hm *model, double Mh, double a, error **err)
{
  ...
  //fac = 3.0*pow(pow(10.0, model->log10Mstar_min)/1.0e10,-0.3);
  fac = 1.0;
...
double *xi_2h(cosmo_hm *model, double a, const double *r, int N, int type, error **err)
{
  ...
  for(i=0;i<N;i++){
    if(r[i] < RMIN2){
      ...
      if (model->hod == berwein02_hexcl || model->hod == leauthaud11 || model->hod == coupon15){
	params.logMlim    = logM_lim(model,a,r[i],err);

	forwardError(*err, __LINE__, NULL);
	params.ngp        = ngal_den(model, a, params.logMlim, model->log10Mstar_min, model->log10Mstar_max, err);
	forwardError(*err, __LINE__, NULL);
	//debugging
...
double int_for_P2h(double logM, void *params, error **err)
{
  ...
  res = dn_dlnM(M, params,err)*b*Ngal(model, M, model->log10Mstar_min, model->log10Mstar_max, err)*rhohat;
  forwardError(*err, __LINE__, 0.0);
...
double log10_fSHMR(double log10Mh, cosmo_hm *model){
  ...
  gsl_spline_eval(spline,log(r),acc);
...
double log10_fSHMR(double log10Mh, cosmo_hm *model){
  /* TO DO: do it by interpolation of inverse function and save table 
    if HOD did not change. */
...
double log10_fSHMR_inv_minus_x(double log10Mstar, void *p){
  ...
  return log10_fSHMR_inv(log10Mstar, model) - model->x;
}
...
double Ngal_c(cosmo_hm *model, double Mh, double log10Mstar_min, double log10Mstar_max, error **err){
  ...
  if (model->hod == coupon15){

    arg    = (log10Mstar_min - log10_fSHMR(log10(Mh), model))/(sqrt(2.0)*sigma_log_M(model,log10Mstar_min, err));
    result = eta_cen(model, Mh, err)*0.5*(1.0-gsl_sf_erf(arg));
    forwardError(*err, __LINE__, 0.0);  
    
    if(log10Mstar_max > 0){
      arg    = (log10Mstar_max - log10_fSHMR(log10(Mh), model))/(sqrt(2.0)*sigma_log_M(model,log10Mstar_max, err));
      result -= eta_cen(model, Mh, err)*0.5*(1.0-gsl_sf_erf(arg));
      forwardError(*err, __LINE__, 0.0);
    }
    ...
    arg    = (log10Mstar_min - log10_fSHMR(log10(Mh), model))/(sqrt(2.0)*model->sigma_log_M);
    result = 0.5*(1.0-gsl_sf_erf(arg));
    forwardError(*err, __LINE__, 0.0);  
    
    if(log10Mstar_max > 0){
      arg    = (log10Mstar_max - log10_fSHMR(log10(Mh), model))/(sqrt(2.0)*model->sigma_log_M);
      result -= 0.5*(1.0-gsl_sf_erf(arg));
      forwardError(*err, __LINE__, 0.0);
    }
    ...
    /* cosmo_hm_params params;
      ...
      result = int_gsl(int_for_Ngal_c, &params, log10Mstar_min, 13.00, 1.0e-3, err);
      if(log10Mstar_max > 0){ 
        result -= int_gsl(int_for_Ngal_c, &params, log10Mstar_max, 13.00, 1.0e-3, err); 
      }
   */
    
  }else{
    arg    = (log10(Mh) - model->log10M_min)/model->sigma_log_M;
    result =  eta_cen(model, Mh, err)*0.5*(1.0+gsl_sf_erf(arg));
    forwardError(*err, __LINE__, 0.0);  
  }
...
double eta_cen(cosmo_hm *model, double Mh, error **err){
   ...
   /* eta must be <= 1 */
  testErrorExit(result > 1.0, GM_ETA, "eta should be smaller than 1.0", *err, __LINE__);
...
double av_Mh_given_Mstar(cosmo_hm *model, double log10Mstar, double a, error **err){
  /* Returns <Mh|M*> */
    // weird behavior below 1e9 -> to check
  double norm, res;
  
  cosmo_hm_params params;
  params.model      = model;
  params.log10Mstar = log10Mstar;
  params.a          = a;
  params.asymptotic = 0;
...
double int_for_phi_c_Mh(double logMh, void *params, error **err){
  ...
  cosmo_hm *model   = ((cosmo_hm_params *)params)->model; 
  double log10Mstar = ((cosmo_hm_params *)params)->log10Mstar;
  double Mh         = exp(logMh);
  
  double res = phi_c_Mstellar(model, log10Mstar, Mh, err)*Mh*dn_dlnM(Mh, params, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_phi_c_Mh_norm(double logMh, void *params, error **err){
  ...
  double res =  phi_c_Mstellar(model, log10Mstar, Mh, err)*dn_dlnM(Mh, params, err);
...
double Ngal_s(cosmo_hm *model, double Mh, double log10Mstar_min, double log10Mstar_max, error **err){
  /*
  ...
  */
  ...
  if (model->hod == coupon15){
    ...
    log10Msat = log10(model->B_sat) + model->beta_sat*log10_fSHMR_inv(log10Mstar_min, model) + 12.0*(1.0-model->beta_sat);
    log10Mcut = log10_fSHMR_inv(log10Mstar_min, model) - 0.5;
    ...
    /* Stellar bin */
    if(log10Mstar_max > 0){
      log10Msat = log10(model->B_sat) + model->beta_sat*log10_fSHMR_inv(log10Mstar_max, model) + 12.0*(1.0-model->beta_sat);
      log10Mcut = log10_fSHMR_inv(log10Mstar_max,  model) - 0.5;
      M0        = pow(10.0, log10Mcut);
      if(Mh - M0 > 0.0){
    	result -= pow((Mh - M0)/pow(10.0, log10Msat), model->alpha);
      }
    ...
    log10Msat = log10(model->B_sat) + model->beta_sat*(log10_fSHMR_inv(log10Mstar_min, model)-12.0) + 12.0;
    log10Mcut = log10(model->B_cut) + model->beta_cut*(log10_fSHMR_inv(log10Mstar_min, model)-12.0) + 12.0;
    result =  Ngal_c(model, Mh, log10Mstar_min, -1, err) * pow(Mh/pow(10.0, log10Msat), model->alpha) * exp(-pow(10.0, log10Mcut)/Mh);
    if(log10Mstar_max > 0){
       log10Msat = log10(model->B_sat) + model->beta_sat*(log10_fSHMR_inv(log10Mstar_max, model)-12.0) + 12.0;
       log10Mcut = log10(model->B_cut) + model->beta_cut*(log10_fSHMR_inv(log10Mstar_max, model)-12.0) + 12.0;
       result   -= Ngal_c(model, Mh, log10Mstar_max, -1, err) * pow(Mh/pow(10.0, log10Msat), model->alpha) * exp(-pow(10.0, log10Mcut)/Mh);
    }
    ...
    */     
    if (model->log10M0 < 0.0) {     /* Negative M0: set to M_min */
      M0 = pow(10.0, model->log10M_min);
    } else {
      M0 = pow(10.0, model->log10M0);
    }
    ...
    result = pow((Mh - M0)/pow(10.0, model->log10M1), model->alpha);
  }
...
double Ngal(cosmo_hm *model, double Mh, double log10Mstar_min, double log10Mstar_max, error **err){
  /* Total number of galaxies per halo.
   */
  double Ngalc, Ngals;
    
  Ngalc = Ngal_c(model, Mh, log10Mstar_min, log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
  
  Ngals = Ngal_s(model, Mh, log10Mstar_min, log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double Mstar_tot_c(cosmo_hm *model, double Mh, double log10Mstar_min, double log10Mstar_max, error **err)
{
  ...
  if(model->hod == leauthaud11 || model->hod == coupon15){
    ...
    res  = int_gsl(int_for_Mstar_tot_c, (void*)&params, log10Mstar_min, log10Mstar_max, 1.e-3, err);
    forwardError(*err, __LINE__, 0.0);
    res -= Ngal_c(model, Mh, log10Mstar_max, -1, err)*pow(10.0, log10Mstar_max) - Ngal_c(model, Mh, log10Mstar_min, -1, err)*pow(10.0, log10Mstar_min);
    forwardError(*err, __LINE__, 0.0);
...
double int_for_Mstar_tot_c(double log10Mstar, void *params, error **err)
{
  ...
  res = Ngal_c(model, Mh, log10Mstar, -1, err) * pow(10.0, log10Mstar)*log(10);
  forwardError(*err, __LINE__, 0.0);
...
double Mstar_tot_s(cosmo_hm *model, double Mh, double log10Mstar_min, double log10Mstar_max, error **err)
{
  ...
  if(model->hod == leauthaud11 || model->hod == coupon15){
    ...
    res  = int_gsl(int_for_Mstar_tot_s, (void*)&params, log10Mstar_min, log10Mstar_max, 1.e-3, err);
    forwardError(*err, __LINE__, 0.0);
    res -= Ngal_s(model, Mh, log10Mstar_max, -1, err)*pow(10.0, log10Mstar_max) - Ngal_s(model, Mh, log10Mstar_min, -1, err)*pow(10.0, log10Mstar_min);
    forwardError(*err, __LINE__, 0.0);
...
double int_for_Mstar_tot_s(double log10Mstar, void *params, error **err)
{
  ...
  res = Ngal_s(model, Mh, log10Mstar, -1, err)*pow(10, log10Mstar)*log(10);
  forwardError(*err, __LINE__, 0.0);
...
double av_gal_bias(cosmo_hm *model, double a, error **err)
{
  ...
  ng = ngal_den(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_av_gal_bias(double logM, void *params, error **err)
{
  ...
  b   = halo_bias(model, M, a, 1, err); 
  res = dn_dlnM(M,params,err)*b*Ngal(model, M, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double av_halo_mass_cen(cosmo_hm *model, double a, error **err)
{
  ...
  ng = ngal_den_c(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);

  res = int_gsl(int_for_av_halo_mass_cen, (void*)&params, min, logMmax, 1.e-5, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_av_halo_mass_cen(double logM, void *params, error **err)
{
  double res;
  double M         = exp(logM);
  cosmo_hm *model  = ((cosmo_hm_params *)params)->model; 
  
  res = M * dn_dlnM(M, params, err) * Ngal_c(model, M, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double av_halo_mass_sat(cosmo_hm *model, double a, error **err)
{
  /* returns the mean halo mass (depends on HOD). */
  ...
  ng = ngal_den_s(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);

  res = int_gsl(int_for_av_halo_mass_sat, (void*)&params, min, logMmax, 1.e-5, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_av_halo_mass_sat(double logM, void *params, error **err)
{
  double res;
  double M         = exp(logM);
  cosmo_hm *model  = ((cosmo_hm_params *)params)->model; 
  
  res = M * dn_dlnM(M, params, err) * Ngal_s(model, M, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double av_halo_mass(cosmo_hm *model, double a, error **err)
{
  ...
  res = int_gsl(int_for_av_halo_mass, (void*)&params, min, logMmax, 1.e-5, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_av_halo_mass(double logM, void *params, error **err)
{
  ...
  res = M * dn_dlnM(M, params, err) * Ngal(model, M, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double av_frsat(cosmo_hm *model, double a, error **err)
{
  ...
  ng_c = ngal_den_c(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
...
double mass_weighted_av_stellar_mass(cosmo_hm *model, double a, error **err){
  ...
  num = int_gsl(int_for_mass_weighted_av_stellar_mass, (void*)&params, model->log10Mstar_min, model->log10Mstar_max, 1.e-3, err);
  forwardError(*err, __LINE__, 0.0);
  
  den = int_gsl(int_for_mass_denum_weighted_av_stellar_mass, (void*)&params, model->log10Mstar_min, model->log10Mstar_max, 1.e-3, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_mass_weighted_av_stellar_mass(double log10Mstar, void *params, error **err)
{
  double res;
  cosmo_hm *model  = ((cosmo_hm_params *)params)->model; 
  double a         = ((cosmo_hm_params *)params)->a; 
  ...
  /* This is M * M * SMF */
  res  = pow(10.0, log10Mstar*2.0) * ngal_den(model, a, logMmax, log10Mstar - dlog10/2.0, log10Mstar + dlog10/2.0, err)/dlog10;
  forwardError(*err, __LINE__, 0.0);

  return res;
}

double int_for_mass_denum_weighted_av_stellar_mass(double log10Mstar, void *params, error **err){
  ...
  res  = pow(10.0, log10Mstar)*ngal_den(model, a, logMmax, log10Mstar - dlog10/2.0, log10Mstar + dlog10/2.0, err)/dlog10;
  forwardError(*err, __LINE__, 0.0);
...
double av_stellar_mass(cosmo_hm *model, double a, error **err)
{
  ...
  res = int_gsl(int_for_av_stellar_mass, (void*)&params, model->log10Mstar_min, model->log10Mstar_max, 1.e-3, err);
  forwardError(*err, __LINE__, 0.0);
...
double int_for_av_stellar_mass(double log10Mstar, void *params, error **err)
{
  double res;
  cosmo_hm *model = ((cosmo_hm_params *)params)->model; 
  double a        = ((cosmo_hm_params *)params)->a; 
...
double logM_lim(cosmo_hm *model, double a, double r, error **err)
{
  ...
  do {
    ...
    ng = ngal_den(model, a, logM, model->log10Mstar_min, model->log10Mstar_max, err);
    forwardError(*err, __LINE__, 0.0);
...
double ngal_triax(cosmo_hm *model, double a, double r,  error **err){
  ...
  params.a          = a;
  params.asymptotic = 0;
  params.log10Mstar_min   = model->log10Mstar_min;
  params.log10Mstar_max   = model->log10Mstar_max;
...
double ngal_den(cosmo_hm *model, double a, double logMlim, double log10Mstar_min, double log10Mstar_max, error **err)
/* ---------------------------------------------------------------- *
 ...
 * ---------------------------------------------------------------- */
{
  ...
  params.asymptotic    = 0;
  params.err           = err;
  params.log10Mstar_min  = log10Mstar_min;
  params.log10Mstar_max  = log10Mstar_max;
...
double int_for_ngal_den(double logM, void *params,  error **err) {
  cosmo_hm *model;
  double res, M, log10Mstar_min, log10Mstar_max;
  
  model        = ((cosmo_hm_params *)params)->model;
  log10Mstar_min = ((cosmo_hm_params *)params)->log10Mstar_min;
  log10Mstar_max = ((cosmo_hm_params *)params)->log10Mstar_max;

  M     = exp(logM);
  
  res   = Ngal(model, M, log10Mstar_min, log10Mstar_max, err)*dn_dlnM(M, params, err);
  forwardError(*err, __LINE__, -1.0);
...
double ngal_den_c(cosmo_hm *model, double a, double logMlim, double log10Mstar_min, double log10Mstar_max, error **err)
/* ---------------------------------------------------------------- *
 * central galaxy number density per unit volume for a given HOD. In h^3 Mpc^-3.
 * logMlim is the maximum halo mass to integrate. log10Mstar_min/max 
 * are the stellar mass bin limit (set log10Mstar_max = -1 for 
 * threshold sample)
 * ---------------------------------------------------------------- */
{
...
double int_for_ngal_den_c(double logM, void *params,  error **err) {
  ...
  res   = Ngal_c(model, M, log10Mstar_min, log10Mstar_max, err)*dn_dlnM(M, params, err);
  forwardError(*err, __LINE__, -1.0);
...
double ngal_den_s(cosmo_hm *model, double a, double logMlim, double log10Mstar_min, double log10Mstar_max, error **err)
/* ---------------------------------------------------------------- *
 * central galaxy number density per unit volume for a given HOD. In h^3 Mpc^-3.
 * logMlim is the maximum halo mass to integrate. Mstellar_min/max 
 * are the stellar mass bin limit (set Mstellar_max = -1 for 
...
double int_for_ngal_den_s(double logM, void *params,  error **err) {
  ...
  res   = Ngal_s(model, M, log10Mstar_min, log10Mstar_max, err)*dn_dlnM(M, params, err);
  forwardError(*err, __LINE__, -1.0);
...
double ngal_weighted(cosmo_hm *model, error **err){
  ...
  for(i=0;i<nz->nbins;i++){ //loop over z
    tmp  = ngal_den(model, 1.0/(1.0+nz->z[i]), logMmax, model->log10Mstar_min, model->log10Mstar_max, err)*nz->n[i];
    forwardError(*err, __LINE__, 0.0);
...
int change_HOD(cosmo_hm *avant, cosmo_hm *apres)
{
  ...
  /************************************/
   ...
   if (NCOCLOSE(avant, apres, log10M_min) || NCOCLOSE(avant, apres, log10M1) ||
       NCOCLOSE(avant, apres, log10M0) || NCOCLOSE(avant, apres, sigma_log_M) ||
...
double Ngal_mean(cosmo_hm *model, double a, error **err)
{
  ...
  double ng = ngal_den(model, a, logMmax, model->log10Mstar_min, model->log10Mstar_max, err);
  forwardError(*err, __LINE__, 0.0);
  
  res = ng/int_gsl(dn_dlnM_integrand,(void*)&params, model->log10M_min*log(10.0),logMmax,1.e-3,err);
  forwardError(*err, __LINE__, 0.0);
  return res;
}
		     ...
		     ngal_fit_t ngal_fit_type, double *ngd, int dologw, error **err)
{
   ...
   if (ngal_fit_type!=ngal_lin_fit_only) {
	 ...
	 for (j=0; j<wth->nbins; j++) {
	   ...
	   testErrorRet(!isfinite(wth->wcov[i*wth->nbins+j]), ce_infnan, "inf or nan in logl", *err, __LINE__, -1.0);
	   chisq   += dchisq1;
  ...
  testErrorRet(!isfinite(dchisq2), ce_infnan, "inf or nan in logl (dchisq2)", *err, __LINE__, -1.0);
  testErrorRet(!isfinite(chisq), ce_infnan, "inf or nan in logl", *err, __LINE__, -1.0);
